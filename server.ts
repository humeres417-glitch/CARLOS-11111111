...
async function startServer() {
  ...
  app.post('/api/drive/upload-inspection', async (req, res) => {
    try {
      ...
      const clientName = (inspection.client?.name || 'Cliente').trim().replace(/[/\\?%*:|"<>]/g, '') || 'Cliente';
      const address = (inspection.client?.address || inspection.client?.comuna || 'Sin Direccion').trim().replace(/[/\\?%*:|"<>]/g, '') || 'Sin_Direccion';
      let date = (inspection.technical?.inspectionDate || '').trim().replace(/[/\\?%*:|"<>]/g, '');
      if (!date) {
        date = inspection.createdAt ? inspection.createdAt.slice(0, 10) : new Date().toISOString().slice(0, 10);
      } else if (date.includes('T')) {
        date = date.split('T')[0];
      }

      const formattedName = `${clientName}_${address}_${date}`;
      const folderName = formattedName;
